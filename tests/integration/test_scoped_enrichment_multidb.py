...
def test_scoped_local_enrichment_isolated_between_two_personal_databases():
    ...
    try:
        ...
        with engines[0].connect() as connection:
            before_b = connection.execute(text(
                "SELECT count(*) FROM resource_statements"
            )).scalar_one()
        _insert_asset(engines[0], scopes[0].id, f"p3d-{token}-a-two")
        result = EnrichmentWorker(
            PostgreSQLObservationRepository(engines[0]),
            FileMetadataExtractor(),
            provider=FileMetadataExtractor.discovery_providers,
        ).run_once(batch_size=20000)
        assert result.failed == 0
        ...
        with engines[1].connect() as connection:
            after_b = connection.execute(text(
                "SELECT count(*) FROM resource_statements"
            )).scalar_one()

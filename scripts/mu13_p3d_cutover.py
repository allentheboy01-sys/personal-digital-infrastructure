...
from pdi.production_ops.enrichment_cutover import (
    ...
    SystemdScopedEnrichmentActions,
)
...
def main(argv=None):
    ...
    try:
        ...
        reader = ProductionEvidenceReader(
            ...
            rollback_metadata=rollback, config=config,
        )
        backend = SystemdScopedEnrichmentActions()
        if args.action == "preflight":
            control.preflight(reader.collect())
            ...
            try:
                ...
                control.activation_result(enabled=False, all_disabled=confirmed)
            else:
                control.activation_result(enabled=True, all_disabled=False)
        elif args.action == "verify":
            evidence = reader.collect()
            control.verify({"p3c_healthy": evidence["writers_healthy"],
                            "p3d_healthy": all(backend.is_enabled(key) and backend.is_active(key)
